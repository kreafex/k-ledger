...
export const BudgetsPage = () => {
  ...
  const loadBudgetData = async (userId) => {
    ...
    // We don't need transactions from last year to calculate this month's budget
    const now = new Date();
    const startOfMonth = new Date(now.getFullYear(), now.getMonth(), 1).toISOString();
    
    const { data: transactions } = await supabase
      .from('transactions')
      .select('amount, category, type') // Fetch only necessary columns
      .eq('type', 'expense')
      .gte('date', startOfMonth); // <--- THIS MAKES IT FAST
